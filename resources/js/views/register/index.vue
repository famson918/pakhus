		...
		<div class="d-flex flex-column flex-column-fluid bgi-position-y-bottom position-x-center bgi-no-repeat bgi-size-contain bgi-attachment-fixed background-image" style="">
			...
			<div class="d-flex flex-center flex-column flex-column-fluid p-10 pb-lg-20">
				...
				<div class="w-lg-600px bg-body rounded shadow-sm p-10 p-lg-15 mx-auto">
					<!--begin::Form-->
					<form @submit.prevent="submitForm" class="form w-100" novalidate="novalidate" id="kt_sign_up_form">
						<!--begin::Heading-->
						...
						<div class="mb-10 fv-row" data-kt-password-meter="true">
							...
							<div class="mb-1">
								...
								<div class="position-relative mb-3">
									<input v-model="registerForm.password"  class="form-control form-control-lg form-control-solid" :type="checkPassword" placeholder="" name="password" autocomplete="off" />
									<div class="text-danger mt-1">
										{{ errors.password }}
									</div>
						...
						<div class="fv-row mb-5">
							...
							<div v-if="!isPasswordConfirmed" class="text-danger mt-1">
								{{ $t('passwordNotConfirmed') }}
							</div>
						</div>
						<!--end::Input group-->
						<!--begin::Input group-->
...
const checkPassword = ref('password');
const passwordVisible = ref(false);
const {  validationErrors, processing, submitRegister } = useAuth();
...
// Example usage
const isPasswordConfirmed = ref(true);

watchEffect(() => {
// This function will run whenever the value of `locale` changes
console.log('Locale changed:', locale.value);
validate();
errors
// Call your custom function here when the locale changes
// Example: myFunctionToRunOnLocaleChange();
});
function submitForm() {
console.log('registerForm.terms :>> ', registerForm.terms);
if (isPasswordConfirmed) {
	validate().then(form => { if (form.valid) submitRegister(registerForm) })
}
}

const showPassword = () => {
passwordVisible.value = !passwordVisible.value;
checkPassword.value = checkPassword.value === 'password'? 'text' : 'password';
}
...
console.log('object :>> ');
isPasswordConfirmed.value = passwordsMatch(registerForm.password, registerForm.password_confirmation);
};

watchEffect(()=> {validatePasswordConfirmation()})
